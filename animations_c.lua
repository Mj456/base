	...
	if tostring(args[1]) == nil then
		...
		if tostring(args[1]) ~= nil then
			...
			elseif argh == 'sit' then
				local ad = "anim@heists@prison_heistunfinished_biztarget_idle"
			
				if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
					if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
						loadAnimDict( ad )
						if ( IsEntityPlayingAnim( player, ad, "target_idle", 3 ) ) then 
							TaskPlayAnim( player, ad, "target_idle", 3.0, 1.0, -1, 49, 0, 0, 0, 0 )
							ClearPedSecondaryTask(player)
							Wait (100)
						else
							TaskPlayAnim( player, ad, "target_idle", 3.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterCommand("ta",function(source, args)
	...
	local ad = "anim@heists@ornate_bank@grab_cash"  --- insert the animation dic here
	local anim = "grab" --- insert the animation name here
